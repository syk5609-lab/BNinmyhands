...
export function CoinDetail() {
  ...
  const symbolNotFound = !invalidTimeframe && !invalidRunId && !validSymbols.includes(upperSymbol);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const t = setTimeout(() => {
      setCoin(generateCoinDetail(upperSymbol));
      setHistory(generateHistory(upperSymbol));
      setLoading(false);
    }, 600);
    return () => clearTimeout(t);
  }, [upperSymbol, timeframe, hasError]);
