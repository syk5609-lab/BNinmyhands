...
import { ArrowUp, ArrowDown, Minus, ChevronRight, Table2 } from 'lucide-react';
...
export function RankingsTable({ data, preset, pumpMode, search, ctx }: Props) {
  ...
  if (filtered.length === 0) {
    return (
      <div className="px-4 sm:px-5 max-w-[1600px] mx-auto">
        <TableSectionHeader label={cfg.label} count={0} sortKey={cfg.defaultSort} />
  ...
  return (
    <div className="px-4 sm:px-5 max-w-[1600px] mx-auto w-full" style={{ fontFamily: 'Inter, sans-serif' }}>
      <TableSectionHeader label={cfg.label} count={filtered.length} sortKey={cfg.defaultSort} />
...
function TableSectionHeader({ label, count, sortKey }: { label: string; count: number; sortKey: keyof CoinData }) {
  return (
