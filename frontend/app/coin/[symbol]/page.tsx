    ...
    <main className="mx-auto max-w-3xl p-8">
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-zinc-300">
          <p>
            Could not load candidate details for <span className="font-medium text-zinc-100">{symbol}</span>. Confirm the
            symbol exists for this timeframe and the backend is running.
          </p>
...
export default async function CoinDetailPage({
  ...
  if (!parsedSymbol || !timeframe) {
    return <UnavailableState symbol={parsedSymbol || "unknown symbol"} />;
  }

  try {
    const [latest, history] = await Promise.all([
      fetchAssetLatest(parsedSymbol, timeframe),
      fetchAssetHistory(parsedSymbol, timeframe, 120),
    ]);
