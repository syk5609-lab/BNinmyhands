...
import { useSearchParams } from '../preview-router';

type LoadPhase = 'loading' | 'error' | 'no_run' | 'ready';

export function Dashboard() {
  const [previewSearchParams] = useSearchParams();
  const previewState = previewSearchParams.get('state') || 'ready';
  const initialTimeframe = (previewSearchParams.get('timeframe') || '4h') as Timeframe;
  ...
  const [ctx, setCtx] = useState<RunContext>(getRunContext(initialTimeframe));
