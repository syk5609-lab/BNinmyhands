...
import { MessageSquare, Send, Shield, Trash2, Flag, LogIn, Loader2, Lock, Users } from '../../lucide';
import { useAuth } from '../../preview-auth';
import { useNavigate } from '../../preview-router';
import { usePreviewCommunityData, type CommunityPost } from '../../preview-community-data';
import { useFeatureFlags } from '../../preview-feature-flags';

function timeAgo(iso: string): string {
  const diff = new Date('2026-03-20T20:52:00.000Z').getTime() - new Date(iso).getTime();
...
function DiscussionInner({ symbol }: { symbol: string }) {
  ...
  const { getPostsForSymbol } = usePreviewCommunityData();
