...
import { ExternalLink, Megaphone } from '../../lucide';
import { useFeatureFlags } from '../../preview-feature-flags';
import { usePreviewCommunityData, type SponsoredAd } from '../../preview-community-data';
...
export function SponsoredSlot({ inline, position = 'dashboard_mid', showDisabledState }: Props) {
  ...
  const { getAdBySlot } = usePreviewCommunityData();
  const enabled = flags.sponsoredAdsEnabled;
  const slotIndex = position === 'dashboard_top' ? 0 : position === 'dashboard_mid' ? 1 : position === 'detail' ? 0 : position === 'detail_bottom' ? 1 : 1;
  const ad = useMemo(() => getAdBySlot(slotIndex), [getAdBySlot, slotIndex]);
