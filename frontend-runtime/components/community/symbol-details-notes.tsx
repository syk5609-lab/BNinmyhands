...
export function SymbolDetailsNotes({ selected }: { selected?: SymbolScanResult }) {
  ...
  const draft = draftByKey[noteKey] ?? "";
  ...
  const saveNote = () => {
    ...
    setDraftByKey((current) => ({ ...current, [noteKey]: "" }));
              ...
              {user ? (
                <>
                  <Textarea
                    ...
                    value={draft}
                    onChange={(e) => setDraftByKey((current) => ({ ...current, [noteKey]: e.target.value }))}
                  />
                ...
                <LoginRequiredCTA
                  ...
                  description="Guest read-only detail access stays available, but personal notes require an account."
                />
              )}
              <div className="space-y-2">
